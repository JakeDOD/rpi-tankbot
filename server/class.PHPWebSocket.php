...
class PHPWebSocket
{
	...
	*/
	...
	function wsStartServer($host, $port) {
		...
		// $nextPingCheck = time() + 1; //OLD TIMING FUNCTION
		$nextPingCheck = microtime(true) + 0.01;	//new timing function to check every 10 milliseconds
		while (isset($this->wsRead[0])) {
			$changed = $this->wsRead;
			//$result = socket_select($changed, $write, $except, 1);	//OLD TIMING
			$result = socket_select($changed, $write, $except, 0, 10000);	//new timing to enable checks every 10 milliseconds
			...
			//OLD TIMING FUNCTION CHECKED EVERY SECOND
			// if (time() >= $nextPingCheck) {
			// 	$this->wsCheckIdleClients();
			// 	$nextPingCheck = time() + 1;
			// }

			//new timing function to check every 10 milliseconds
			if (microtime(true) >= $nextPingCheck) {
				$this->wsCheckIdleClients();
				$nextPingCheck = microtime(true) + 0.01; //must be changed for millisecond precision
			}
		}
